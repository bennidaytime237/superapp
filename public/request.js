...
function renderPicker(rows){
  ...
  const items = rows.map(({c,ci,t,ti}) =>
    Safe.html`<button data-ti="${ti}" data-ci="${ci}" class="pick-btn w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-surface-container-low transition-colors text-left">
      ...
      <div class="flex-1"><p class="font-bold text-sm">${t.symbol}${t.native?Safe.html`<span class="ml-1 px-1 rounded-md bg-surface-container-high text-xs font-medium text-on-surface-variant align-middle">Gas Token</span>`:Safe.html``}</p><p class="text-xs text-on-surface-variant">${t.name} · ${c.name}</p></div>
    </button>`
  );
